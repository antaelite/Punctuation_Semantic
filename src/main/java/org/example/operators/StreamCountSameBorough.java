...
public class StreamCountSameBorough extends PunctuatedIterator {
    ...
    // Map: Borough -> Number of trips
    private transient MapState<String, Integer> count;

    @Override
    public void open(Configuration parameters) {
        GeoUtils.loadBoroughs("src/main/resources/nyc-boroughs.geojson");
        count = getRuntimeContext().getMapState(new MapStateDescriptor<>("CountSame", String.class, Integer.class));
    ...
    @Override public void pass(Punctuation p, Context ctx, Collector<StreamItem> out) throws Exception {
        for (String b : count.keys()) {
            out.collect(new BoroughCountItem(b, count.get(b)));
        }
    }
